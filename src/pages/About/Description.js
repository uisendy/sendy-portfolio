...
const Description = () => {
  ...
  const coreValues = coreValuesObj.map((coreValue) => (
    <li
      key={coreValue.id}
      className="font-montserrat py-5 px-6 border-y grid grid-cols-6 md:grid-cols-4 items-center gap-6 font-light text-4xl"
    >
      <p className="text-3xl md:text-5xl lg:text-7xl text-left">{coreValue.no}</p>
      <p className=" text-lg lg:text-2xl col-span-5 md:col-span-3 ">{coreValue.value}</p>
    </li>
  ));

  return (
    <div data-scroll-section className=" mb-32 px-[6.5%]">
      ...
      <div className="description__section md:h-[70vh] grid lg:grid-cols-2 items-center ">
        <div className="flex flex-col gap-8 ">
          <p className=" text-lg lg:text-2xl pr-0 lg:pr-[10%] leading-relaxed font-montserrat font-light pb-10 lg:pb-0">
